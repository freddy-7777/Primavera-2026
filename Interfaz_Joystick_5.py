...
esp32_ip = "192.168.137.124"  # Asegúrate de que esté en la misma red
...
# -------- Leer Joystick por WiFi --------
def leer_joystick():
    try:
        r = requests.get(f"http://{esp32_ip}/joystick", timeout=1)
        data = r.text.strip()
        ...
        label_sw.config(
            text="Botón: PRESIONADO" if sw == "0" else "Botón: LIBRE"
        )

    except:
        label_x.config(text="X: ---")
        label_y.config(text="Y: ---")
        label_sw.config(text="Botón: ---")

    # Llamarse a sí misma cada 200 ms
    cuadro.after(200, leer_joystick)
...
# Labels Joystick
label_x = tk.Label(ButtonsFrame, text="X: ---", font=("Arial", 14),
                   bg="blue", fg="white")
label_x.place(x=700, y=200)

label_y = tk.Label(ButtonsFrame, text="Y: ---", font=("Arial", 14),
                   bg="blue", fg="white")
label_y.place(x=700, y=240)

label_sw = tk.Label(ButtonsFrame, text="Botón: ---", font=("Arial", 14),
                    bg="blue", fg="white")
label_sw.place(x=700, y=280)

# Iniciar lectura del joystick
leer_joystick()
